...
func newCleanupCmd(c *common) *cobra.Command {
	cmd := &cobra.Command{Use: "cleanup", Short: "Emit (stdout only) a delete-by-query for records indexed before a date", Args: cobra.NoArgs}
	fs := cmd.Flags()
	qf := &queryFlags{}
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		text, err := renderCleanupCommand(idx.Server, q, n)
		if err != nil {
			return err
		}
